...
- `users().get_pricing()` for looking up product pricing, including standard
  (non-premium) domain register / renew / transfer prices per TLD and duration
  (`namecheap.users.getPricing`). Use `PricingResult::prices()` to flatten the
  nested response to per-duration price entries.
